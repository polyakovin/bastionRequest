...
(function() {
  ...
  $(document).ready(function() {
    var left_bar, left_bar_trigger, order;
    ...
    $("form").on('keyup change', function() {
      bastion.set_values(order, bastion.default_order);
      bastion.insert_data(order);
      return $(document).prop('title', "Заказ № " + order.prenumber + "–" + order.number);
    });
    ...
    left_bar = $(".left_bar");
    return left_bar_trigger.click(function() {
      if (left_bar.css("left") === "0px") {
        return left_bar.animate({
          left: -315
        }, 200);
      } else {
        return left_bar.animate({
...
(function() {
  var default_order, h, inAWeek, rusDate, tday, to_literal, today;
  ...
  module.exports = {
    ...
    set_defaults: function() {
      ...
      return $("#i_date_of_install").val(inAWeek);
    },
    insert_data: function(order) {
      var addcost, cost, delivery, fio, fio_s, full_number, hinge, hinge_2, i_zadv, maincost, opening_type, rest_cost, side, stvorka, type, zadv;
      fio = order.family + " " + order.name + " " + order.father_name;
      fio_s = order.family + " " + order.name[0] + "." + order.father_name[0] + ".";
      if (+order.opening_type === 0) {
      ...
      $("input[name='type']").on('keyup change', function() {
        ...
        if (+order.type === 1) {
          i_zadv.val("СТРАЖ");
          return zadv.html("СТРАЖ");
        } else {
          i_zadv.val("СТРАЖ + Торцевые");
          return zadv.html("СТРАЖ + Торцевые");
        }
      });
      if (order.construction.toLowerCase() === "дмп") {
        $(".if_dmp").html(" противопожарная глухая");
        return $(".data-construction").html("ДМП–Г–" + order.type + "/ЕI-60");
      } else {
        $(".if_dmp").html("");
        return $(".data-construction").html(order.construction);
      }
    },
    set_values: function(order, default_order) {
      order.prenumber = $("#i_prenumber").val() || default_order.prenumber;
      order.number = $("#i_number").val() || default_order.number;
      order.family = $("#i_family").val() || default_order.family;
      order.name = $("#i_name").val() || default_order.name;
      order.father_name = $("#i_father_name").val() || default_order.father_name;
      order.address = $("#i_address").val() || default_order.address;
      order.tel_1 = $("#i_tel_1").val() || default_order.tel_1;
      order.tel_2 = $("#i_tel_2").val() || default_order.tel_2;
      order.date_of_order = $("#i_date_of_order").val() || default_order.date_of_order;
      order.date_of_install = $("#i_date_of_install").val() || default_order.date_of_install;
      order.type = $("input[name='type']:radio:checked").val();
      order.side = $("input[name='open_side']:radio:checked").val();
      order.width = $("#i_width").val() || default_order.width;
      order.height = $("#i_height").val() || default_order.height;
      order.delivery = $("input[name='delivery']:radio:checked").val();
      order.cost = $("#i_cost").val() || default_order.cost;
      order.prepayment = $("#i_prepayment").val() || default_order.prepayment;
      ...
      order.construction = $("#i_construction").val() || default_order.construction;
      order.opening_type = $("input[name='opening_type']:radio:checked").val();
      order.otd_out = $("#i_otd_out").val() || default_order.otd_out;
      order.otd_in = $("#i_otd_in").val() || default_order.otd_in;
      order.nalich = $("#i_nalich").val() || default_order.nalich;
      order.glaz = $("#i_glaz").val() || default_order.glaz;
      order.zadv = $("#i_zadv").val() || default_order.zadv;
      order.lock_top = $("#i_lock_top").val() || default_order.lock_top;
      order.lock_bottom = $("#i_lock_bottom").val() || default_order.lock_bottom;
      order.furnitura = $("#i_furnitura").val() || default_order.furnitura;
      order.lich = $("#i_lich").val() || default_order.lich;
      order.bronenakl = $("#i_bronenakl").val() || default_order.bronenakl;
      order.warmer = $("#i_warmer").val() || default_order.warmer;
      return order.notes = $("#i_notes").val() || default_order.notes;
    }
  };

}).call(this);

},{"./helpers.js":4,"./rusDate.js":5,"./to_literal.js":6,"jquery":8}],4:[function(require,module,exports){
(function() {
  module.exports = {
...
},{"./helpers.js":4}],6:[function(require,module,exports){
(function() {
